...
If someone says ``Anna ate some cookies'', the hearer of the sentence might infer the upper-bounded reading that Anna ate \textit{some, but not all} cookies. Similarly, given ``Donald ate a donut or a pretzel.'', one might infer that Donald ate either \textit{the donut or the pretzel, but not both} (i.e., an exclusive interpretation). 
...
We investigate the effects of three factors that have been argued to influence the robustness of SIs: (1) \textit{relevance} of the stronger alternative to the listener, (2) the \textit{competence} of the speaker about the truth of the stronger alternative, and (3) the \textit{prior probability} that the stronger alternative is true [1,3,5]. Critically, we explore how these three factors \textit{interactively} influence the robustness of SIs associated with the triggers ``some'' and ``or''. %Simultanously, we directly compare whether the two triggers are both influenced by these factors.
...
Given the unexpected absence of prior effects for ``or'', we computed exploratory pairwise correlations of all the predictors. While no correlations were found for ``some'', we found a significant correlation between the prior and relevance effects for ``or'' ($R^2 = -0.106, p<0.01$). %, and a significant correlation between competence and relevance ($R^2 = 0.127, p=0.001$) for ``or''. 
