...
Below, we conduct a similar grid search over more models. More specifically, we compute the BF in favor of a more complex model over the intercept-only baseline model. We compute these pairwise comparisons for each iteratively added explaantory factor. This results in 7 model comparisons. For each comparison, we perform a small grid search over the SD parameter of the prior t-distribution over the fixed effects regression coefficients, using the values [0.05, 0.1, 0.15]. For each model, 40.000 samples are extracted.
```{r}
# define a convenience function for iterating over the SDs and critical models
models_grid_search_SD_full = function(sd_vec, iter, trigger) {
  # create output df
  results <- tribble(
    ~"model in favor", ~"model against", "~trigger", ~"SD", ~"BF"
  )
  ...
  for (i in sd_vec) {
    ...
    # fit the single predictor models
    ...
    # fit prior * comp model with specific prior
    model_pri_comp <- brm(target ~ prior*competence +
                   (1 + prior + competence || submission_id) +
                   (1 | title),
                 data = data,
                 prior = prior,
    ...
    # rel*pri model
    model_rel_pri <- brm(target ~ prior*relevance +
    ...
    # fit full model
    model_pri_rel_comp <- brm(target ~ prior*competence*relevance +
                   (1 + prior + competence + relevance || submission_id) +
                   (1 | title),
                 data = data,
                 prior = prior,
    ...
    model_list <- c("model_rel", "model_pri", "model_comp", "model_rel_comp", "model_pri_comp", "model_rel_pri", "model_pri_rel_comp")
    for (m in model_list) {
      bf <- bayes_factor(m, model_int, silent = TRUE)
      print(paste("SD: ", i, sep=""))
      print(paste("Model in favor: ", m, sep=""))
      print(paste("BF: ", bf, sep=""))
      # append to results dataframe, write out
      results <- results %>% add_row(
        "model in favor" = m,
...
#ggsave("BF_vs_SD_all_vs_int.pdf", width = 8, height = 4)
```
