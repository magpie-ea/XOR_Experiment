...
# add correctly filtered dataframes 
d_critical_noDuplicates_zScored <- read_csv("./../../data/main/results_prereg_tidy_noDuplicates_zScored_long.csv")
d_critical_noDuplicates_zScored_wide <- read_csv("./../../data/main/results_prereg_tidy_noDuplicates_zScored_wide.csv")
d_critical_noImplFails_noDuplicates_zScored <-
  read_csv("./../../data/main/results_prereg_tidy_noDuplicates_noImplFails_zScored_long.csv")
d_critical_noImplFails_noDuplicates_zScored_wide <-
  read_csv("./../../data/main/results_prereg_tidy_noDuplicates_noImplFails_zScored_wide.csv")
...
Now perform the main confirmatory analysis on correctly z-scored data (no duplicates), to double-check that the results don't change qualitatively.
```{r, results='hide', message=FALSE, warning=FALSE}
# Full model 
d_critical_noDuplicates_zScored_wide <- d_critical_noDuplicates_zScored_wide %>% mutate(
  main_type = as.factor(main_type)
)
# some = 0, xor = 1
contrasts(d_critical_noDuplicates_zScored_wide$main_type) 

# set priors
priors <- set_prior("student_t(1, 0, 2)", class = "b")

model_SI_no_dupl <- brm(target ~ prior*competence*relevance* main_type +
                   (1 + prior + competence + relevance + main_type || submission_id) +
                   (1 | title),
                 data = d_critical_noDuplicates_zScored_wide,
                 prior = priors,
                 sample_prior = T,
                 control = list(adapt_delta = 0.95),
                 cores = 4,
...
```{r}
summary(model_SI_no_dupl)
```

Extract contrasts:
```{r}
model_SI_no_dupl %>% spread_draws(b_Intercept, b_prior, b_competence, b_relevance,
                          b_main_typexor, `b_prior:competence`,
                          `b_prior:relevance`, `b_competence:relevance`,
                          `b_prior:main_typexor`, `b_competence:main_typexor`,
                          `b_relevance:main_typexor`, `b_prior:competence:relevance`, 
  ...
  mutate(
    ...
    relevance_xor =  b_relevance + `b_relevance:main_typexor`,
    relevance_some = b_relevance
  ) -> model_SI_no_dupl_posteriors

# check P that the effects are positive / negative / no effect present
posterior_hypotheses_no_dupl <- model_SI_no_dupl_posteriors %>% 
  select(prior_xor, prior_some, 
         competence_xor, competence_some,
         relevance_xor, relevance_some) %>%
  gather(key, val) %>%
  group_by(key) %>% mutate(positive = mean(val > 0.05),
                           negative = mean(val < -0.05),
                           no = mean(val %>% between(-0.05, 0.05))) %>%
  summarise(negative_eff = mean(negative),
            no_eff = mean(no),
            positive_eff = mean(positive)
            )
posterior_hypotheses_no_dupl

# write out for reading values in paper
#posterior_hypotheses_no_dupl %>% write_csv("./../../writing/paper/data/posterior_h_confirmatory.csv")
```
Also check if the 0 ROPE results change on the correctly z-scored data:
```{r}
posterior_hypotheses_nullROPE_no_dupl <- model_SI_no_dupl_posteriors %>% 
  select(prior_xor, prior_some, 
         competence_xor, competence_some,
         relevance_xor, relevance_some) %>%
  gather(key, val) %>%
...
posterior_hypotheses_nullROPE_no_dupl
#posterior_hypotheses_nullROPE_no_dupl %>% write_csv("./../../writing/paper/data/posterior_h_confirmatory_0-ROPE.csv")
```

Now investigate if the confirmatory results change if excluding participants who were resilient to contextual manipulations:
```{r, results='hide', message=FALSE, warning=FALSE}
# Full model 
d_critical_noImplFails_noDuplicates_zScored_wide <- d_critical_noImplFails_noDuplicates_zScored_wide %>% mutate(
  main_type = as.factor(main_type)
)
# some = 0, xor = 1
contrasts(d_critical_noImplFails_noDuplicates_zScored_wide$main_type) 

# set priors
priors <- set_prior("student_t(1, 0, 2)", class = "b")

model_SI_no_dupl_no_impl <- brm(target ~ prior*competence*relevance* main_type +
                   (1 + prior + competence + relevance + main_type || submission_id) +
                   (1 | title),
                 data = d_critical_noImplFails_noDuplicates_zScored_wide,
                 prior = priors,
                 sample_prior = T,
                 control = list(adapt_delta = 0.95),
                 cores = 4,
...
```{r}
summary(model_SI_no_dupl_no_impl)
```

Contrasts:
```{r}
model_SI_no_dupl_no_impl %>% spread_draws(b_Intercept, b_prior, b_competence, b_relevance,
                          b_main_typexor, `b_prior:competence`,
                          `b_prior:relevance`, `b_competence:relevance`,
                          `b_prior:main_typexor`, `b_competence:main_typexor`,
                          `b_relevance:main_typexor`, `b_prior:competence:relevance`, 
  ...
  mutate(
    ...
    relevance_xor =  b_relevance + `b_relevance:main_typexor`,
    relevance_some = b_relevance
  ) -> model_SI_no_dupl_no_impl_posteriors

# check P that the effects are positive / negative / no effect present
posterior_hypotheses_no_dupl_no_impl <- model_SI_no_dupl_no_impl_posteriors %>% 
  select(prior_xor, prior_some, 
         competence_xor, competence_some,
         relevance_xor, relevance_some) %>%
  gather(key, val) %>%
  ...
  summarise(positive_eff = mean(positive),
            negative_eff = mean(negative),
            no_eff = mean(no))
posterior_hypotheses_no_dupl_no_impl
```

0 ROPE:
```{r}
posterior_hypotheses_nullROPE_no_dupl_no_impl <- model_SI_no_dupl_no_impl_posteriors %>% 
  select(prior_xor, prior_some, 
         competence_xor, competence_some,
         relevance_xor, relevance_some) %>%
  gather(key, val) %>%
...
posterior_hypotheses_nullROPE_no_dupl_no_impl
```
...
# applied to correctly z-scored data
test_conjunction_of_all_hypotheses(posterior_hypotheses_no_dupl)
test_conjunction_of_all_hypotheses(posterior_hypotheses_no_dupl_no_impl)
...
# model_SI_posteriors
model_SI_no_dupl_posteriors %>% select(prior_xor, prior_some,
                               competence_xor, competence_some,
                               relevance_xor, relevance_some) %>%
...
```{r, results='hide', message=FALSE, warning=FALSE}
#d_critical_zScored %>% 
d_critical_noDuplicates_zScored %>%
  select(submission_id, title, main_type, response_centered, prior_class, class_condition, block_extended) %>% # ,
  unique() %>% 
  pivot_wider(
    names_from = class_condition, 
    values_from = prior_class
  )  -> d_critical_wide_cat_zScored 

# wrangle prior categorization of the items for the correctly z-scored data 
d_critical_wide_cat_zScored %>%
  replace_na(list(relevance = 0, competence = 0, prior = 0)) %>%
  group_by(submission_id, title) %>%
  filter(block_extended != "target") %>%
  summarise_at(c( "relevance", "competence", "prior"), funs(sum)) %>%
  mutate(prior = ifelse(prior == 2, 1, prior)) -> d_critical_wide_cat_zScored_priors

d_critical_wide_cat_zScored_targets <- d_critical_wide_cat_zScored %>%
  select(submission_id, title, main_type, block_extended, response_centered) %>%
  filter(block_extended == "target") %>% select(-block_extended)

d_critical_wide_cat_zScored_full <- left_join(d_critical_wide_cat_zScored_priors, d_critical_wide_cat_zScored_targets, by = c("submission_id", "title") )

d_critical_wide_cat_zScored_full %>%
  mutate(relevance = factor(relevance, levels = c(1, 0)),
         competence = factor(competence, levels = c(1, 0)),
         prior = factor(prior, levels = c(1, 0)),
         main_type = as.factor(main_type)) -> d_critical_wide_cat_zScored #%>% 
...
#posterior_hypotheses_cat %>% write_csv("./../../writing/paper/data/posterior_h_exploratory_categorical.csv")
```
...
#ggsave("../../writing/paper/images/posterior-effects-categorical-noDupl.pdf", width=7, height=5)
```
...
```{r, results='hide', message=FALSE, warning=FALSE}
d_critical_zScored_wide_byItem <- d_critical_noDuplicates_zScored_wide %>% ungroup() %>%
...
#posterior_hypotheses_means %>% write_csv("./../../writing/paper/data/posterior_h_exploratory_means.csv")
```
...
#ggsave("../../writing/paper/images/posterior-effects-means-noDupl.pdf", width=7, height=5)
```

[NOTE: Analyses below are not replicated on data z-scored correctly.]
...
model_fake_posteriors_long %>%
  ...
  stat_halfeye(aes(y = effect, x = value, 
               ...
               alpha = .7) +
  geom_vline(xintercept = c(-.05, .05), linetype = "dashed") +
  scale_fill_manual(name = "Direction of effect", values = c("coral", "mediumseagreen")) + # , "yellow3"
  labs(x = "Effect size", y = "Effect") +
  theme(legend.position = "top",
        legend.title = element_text(size = 12),
        legend.text = element_text(size = 12),
...
#ggsave("../../writing/paper/images/posterior-effects-idealized-sim.pdf", width=7, height=5)
```